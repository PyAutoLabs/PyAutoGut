...
- library-pr: PyAutoFit#1436 (OPEN, pending-release)
- phases: 1 (PyAutoFit: quick_update_message + JAX compile message at first call) PR OPEN #1436; 2 (drop the now-duplicated sentence from 23 workspace scripts + regenerate notebooks/markdown) = follow-up issue, prompt at draft/docs/autolens_workspace/sampler_cli_output_workspace_sweep.md
- note: scope grew during verification (human-approved). The cosmetic fix exposed that ONLY Nautilus forwarded iterations_per_quick_update to Fitness — 6 other searches silently never quick-updated; wiring them up then exposed a latent `.tolist()` crash on Dynesty's list-typed parameters. Both fixed here, both with detectors proven to fail on the pre-fix source
