...
## interferometer-jax-jit
- issue: https://github.com/PyAutoLabs/PyAutoArray/issues/422
- status: awaiting-merge
- library-pr: https://github.com/PyAutoLabs/PyAutoArray/pull/423
- prompt: draft/bug/autoarray/interferometer_simulator_jax_jit.md
- scope: interferometer simulator `@jax.jit` path now works for BOTH TransformerDFT and TransformerNUFFT. ONE file, +56/-1. (1) `_register_interferometer_pytrees` — Interferometer (data+noise_map dynamic; uv_wavelengths, real_space_mask, transformer, grids, over-sample sizes, Nones as aux) PLUS Visibilities and VisibilitiesNoiseMap, which nothing else registers (Array2D is already registered on the imaging path, these are not) so they surfaced as bare leaves. (2) `via_image_from` called `transformer.visibilities_from(image=image)` with NO `xp=xp` — one line
- issue-was-wrong: #422 predicted blocker 2 was `TransformerNUFFT._forward_native` hard-converting to NumPy and needing "restructuring with its own correctness surface". **FALSE.** `_forward_native` already has a complete jittable JAX branch (lax.scan + dynamic_slice). The failure at transformer.py:660 was in the NUMPY branch — traceback pointed at the symptom, cause was one frame up in a caller that never threaded xp. transformer.py deliberately UNTOUCHED. Had I edited it as the issue proposed I would have restructured working code to fix a bug elsewhere
- dft-passed-by-luck: TransformerDFT worked despite the missing xp because its arithmetic flows through tracers; only NUFFT (which calls _nufftax then converts) exposed it. A green DFT was NOT evidence the path was threaded
- ship-evidence: DFT and NUFFT numpy-eager vs jax-jit agree ~1e-11 on real AND imaginary parts; BOTH NUFFT chunk branches under jit match NumPy ~5e-13 (single-shot + lax.scan — `chunk_size` is a TransformerNUFFT.__init__ arg the simulator NEVER sets, so that branch is unreachable via the simulator and was exercised on the transformer directly rather than skipped); returned Interferometer keeps uv_wavelengths/real_space_mask/noise_map; NumPy path still ndarray-backed; test_autoarray 929 / test_autogalaxy 1009 / test_autolens 488 — all IDENTICAL to pre-change
- out-of-scope: TransformerNUFFTPyNUFFT (legacy pynufft, not JAX-traceable, not expected to be)
- docs-followup: interferometer `__JAX Variant__` in both workspaces still says the jitted wrap does not work (#379), and autolens interferometer/simulator.py's "TransformerNUFFT supports jax.jit" claim is unqualified. After merge, do for interferometer what #381 did for imaging — restore the recipe, uncomment the call, add to smoke_tests.txt
- origin: 6th and final generation of the arc that began with likelihood-function-jax-pointer (autolens_workspace#368)
- worktree: ~/Code/PyAutoLabs-wt/interferometer-jax-jit
- repos:
  - PyAutoArray (feature/interferometer-jax-jit)
