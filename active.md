...
- status: awaiting-merge
- workspace-pr: https://github.com/PyAutoLabs/autolens_workspace/pull/376 (OPEN, pending-release, head 3dc5058e, CI 4/4 GREEN)
- ci-fallout: the first push went RED on `navigator / Navigator paths + banner lint` with 5 missing refs in files this PR does NOT touch. Root cause: PyAutoHands#213 ("gate relative folder references in README prose") merged 08:28 UTC, BETWEEN autolens_workspace main's last green navigator run (08:21, f7d7884d) and this PR's first (08:32) — this branch was the first to run under the stricter checker. Cloning main and running the checker under the CI root layout reproduces the identical 5 findings, so main was red too. Fixed in commit 3dc5058e: the 5 are literal `autolens_workspace/scripts/...` refs where the repo convention is the wildcard `autolens_workspace/*/...` form the gate tolerates; scripts/README.md refers to its own folder so the prefix is dropped
- navigator-root-name-trap: check_navigator.py `normalise()` strips a leading `<root.name>/` prefix, and CI clones the workspace as `workspace/`, so an `autolens_workspace/`-prefixed LITERAL can never be stripped there. Running `check_navigator.py --root autolens_workspace` locally PASSES on exactly the content CI rejects. Reproduce CI by cloning into a dir literally named `workspace` and passing `--root workspace`. Worth a PyAutoHands follow-up
