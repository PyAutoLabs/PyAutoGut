...
- workspace-pr: https://github.com/PyAutoLabs/autolens_workspace/pull/415, https://github.com/PyAutoLabs/autogalaxy_workspace/pull/195
- summary: Consolidated the six trailing `__JAX__` guide sections (~536 lines, cross-referencing in a ring) into one `scripts/guides/using_jax.py` per workspace, and made the consolidated guide a RUNNABLE script: migrated recipes are executed code cells, `guides/using_jax.py` added to `smoke_tests.txt` (al 34→35, ag 14→15) with an `__Env__` `ENV: jax` declaration releasing the smoke profile's `PYAUTO_DISABLE_JAX=1` (token map in PyAutoHands `env_config.py`; `derive_jax_markers` is off in smoke profiles, so the declaration is required). CI smoke logs on both PRs show `[PASS] guides/using_jax.py` — the #368 prose-rot failure mode is now structurally closed for this material. Host guides keep a `__Contents__`-first `__JAX__` bullet + host-specific pointer paragraph (likelihood_function.py format). Probing extended the prompt's verified findings: the bound-method form `jax.jit(method)(grid=grid, xp=jnp)` fails like the traced-grid form (grid is still a traced argument — lens_calc "Form 2" and both galaxies.py closure recipes were broken as published); `al.LensCalc(deflections_yx_2d_from=...)` takes callables and returns raw `jax.Array` under `xp=jnp` (no `.array` unwrap — wrap behaviour varies per method); the chained-call `ValueError` guard verified firing. Ten sibling scripts' JAX pointers at the deleted sections were retargeted (sweep-the-siblings). `tracer.py`'s stray `Fin.` (missed by #384 — nonstandard spelling) removed. Unattributed speedup figures dropped for a qualitative sentence + `jax_profiling/` pointer. Model: Fable (recorded human override of the tutorial-prose default); one task, one PR per repo (recorded override of the Feature Agent's 4-phase split). Heart YELLOW acknowledged at ship (manifest-drift + staleness, unrelated). Follow-up filed: `draft/docs/autoarray/jax_guard_error_pointer_retarget.md` — the PyAutoArray guard message points at the deleted lens_calc section (gated on #415, now unblocked). al#415 trued up pre-existing notebook drift in `notebooks/group/start_here.ipynb`.
