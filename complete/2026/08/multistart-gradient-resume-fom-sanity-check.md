...
- follow-up (unclaimed): PyAutoFit#1472's `n_value_nan_lane_steps` / `n_grad_nan_lane_steps` counters restore via `search_internal.get(..., 0)` and are designed to keep accumulating across a resume. That is now demonstrable end-to-end for the first time (this bug blocked it), but the verification run here was a clean Gaussian fit with zero NaN lanes, so the accumulation itself is still covered only by unit tests over hand-built `search_internal` dicts. Demonstrating it needs a deliberately NaN-producing analysis.
