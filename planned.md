...
- summary: build the numerical-hazard profiling instrument + results convention in `autolens_profiling`. Phase 1 = the framework in `scripts/misc/hazards/` plus a VERTICAL SLICE of four detectors, one per subject shape: (1) component/saturation (`ell_comps` clamp), (2) component/non-finite-gradient (radial `sqrt` at r=0 — the measure-zero case), (3) component/backend-divergence (`PowerLaw` `hyp2f1` vs 20-term series), (4) matrix/conditioning-floor (the `1.0e-3` curvature-diagonal add from synthetic matrices). Plus the regenerated `ell_comps` seed result, a `--check` regression mode and `hazards_index.json`. Does NOT run the full `component x backend` matrix and does NOT fix any source-library finding.
- architecture: detectors ALL live in `scripts/misc/hazards/` regardless of subject; `scripts/<dataset>/hazards/` holds dataset-specific cells and fixtures only. TIER IS METADATA ON A FINDING, not a directory — subject scope is `component` (a profile, no data), `matrix` (synthetic matrices, no dataset) or `likelihood` (a real dataset). The `matrix` scope exists because `reconstruction_positive_only_from` takes `data_vector`/`curvature_reg_matrix` directly (`inversion_util.py:275`), so a dataset is needed to judge a floor's SCIENTIFIC RELEVANCE, not to DETECT the mechanism. The linear algebra is still tied to the whole likelihood function where it means something — the `likelihood` subject in `scripts/imaging/hazards/`. Matches the repo's dataset-first/task-second layout law.
- REVISION 2 (2026-08-13, after adversarial Codex review returned RETHINK; all 7 factual claims confirmed, design reworked): (a) TYPED MEASUREMENTS replace the universal predicate — risk basis is one of `prior_mass` / `epsilon_neighbourhood` / `reachability` / `error_curve`, because MC prior volume is meaningless for measure-zero gradient sites AND for the `0.99999` clamp, whose region spans only ~5e-6 of ellipticity magnitude (revision 1's own acceptance test contradicted its design). (b) REACHABILITY IS FIRST-CLASS — `EllProfile.__init__` calls `validate_ell_comps` at construction (`geometry_profiles.py:237`), so the `ell_comps` clamp is NOT reachable on the public numpy path at all while JAX tracing skips the guard and reaches it; revision 1's numpy-only acceptance command would have found nothing. Findings record `code_exists`/`reachable_via`/`blocked_by`/`affects_science` separately. (c) phase 1 re-cut as a vertical slice instead of framework-first around the one easy shape. (d) a CONSUMER LOOP added, else this merges as a framework with one demo and never runs again. (e) anchors key on a stable semantic finding ID re-verified by re-running the reproducer — a whole-function sha256 breaks on a comment edit and the phase-2 `general.yaml:7` config finding has no enclosing function at all. (f) dashboard correction: the three `build_readme.py` edits give a PACKAGE-LOCAL table only; `_render_headline()` (`:478`) hard-codes runtime+breakdown, so the root dashboard needs a fourth edit.
