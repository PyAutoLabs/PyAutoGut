...
- blocked-by: clean-output release re-validation (wheel-based mode=release); does NOT reproduce on
  current main
- summary: |
    Phase 1 gate ran all NINE scripts (2 autofit_workspace cookbooks + 7 autofit_workspace_test
    database/output) on current main from a moved-aside output/ under each workspace's
    config/build/profile_release.yaml, env resolved via autohands.env_config.build_env_for_script
    at workspace CWD. 0/9 reproduce. Passes audited, not assumed: 20 fresh searches started, zero
    stale resumes, `assert len(agg) > 0` executed and held, 4 Emcee searches ran with no
    "Probability function returned NaN" anywhere. Same conclusion as the sibling
    samples_parameter_paths (#1327) from this SAME release run — stale cached output/ in the run,
    not a library defect. Two honest limits: cookbooks ran at TEST_MODE=1 (what their release
    profile pins, but fewer emcee proposals), and this was a SOURCE-tree run, not the TestPyPI
    wheels the release run installed. Full evidence: PyAutoFit#1508 comment.
- incidental-findings: scrape's `_add_files_fit` bare `except AttributeError`
    (autofit/database/aggregator/scrape.py:150-158) warns "Failed to load latent variables" for
    essentially every scraped fit — the fit zips contain no latent file at all, so it is warning
    about legitimate absence. CLI-noise hygiene item, NOT a regression. Same guard silently warns
    "Failed to load samples" for multi-analysis child fits. Plus 6x "Could not save covariance
    matrix". File via /hygiene if wanted.
- affected-repos:
  - PyAutoFit
  - autofit_workspace
